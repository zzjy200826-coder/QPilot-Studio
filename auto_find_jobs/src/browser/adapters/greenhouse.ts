...
export class GreenhouseAdapter implements SiteAdapter {
  ...
  async openApply(page: Page, job: DiscoveredJob, _profile?: CandidateProfile): Promise<void> {
    ...
    const existingForm = await page.locator("form input, form textarea, form select").count();
    if (existingForm > 0) {
      return;
    }

    const applyButton = await resolveFirstVisible([
      page.locator("a:has-text('Apply for this job')"),
      page.locator("button:has-text('Apply for this job')")
    ]);
    if (!applyButton) {
      return;
    }
  ...
  async fill(
    ...
    if (continueButton) {
      ...
      await waitForSettledUi(page);
      await onStep?.("已进入 Greenhouse 下一步。");
  ...
  async submit(page: Page): Promise<{ confirmed: boolean; message: string }> {
    ...
    if (!button) {
      return {
        confirmed: false,
        message: "在 Greenhouse 页面上未找到可见的提交按钮。"
      };
    }
